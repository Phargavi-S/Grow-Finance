...
const connectDB = async () => {
  try {
    ...
    try {
      ...
      for (const indexName of legacyIndexes) {
        if (existingIdx.some(i => i.name === indexName)) {
          try {
            ...
            console.warn(` Could not drop ${indexName} index:`, dropErr.message);
          }
        }
      }
      ...
      if (hasFirebaseIndex) {
        try {
          ...
          console.warn(' Could not drop firebaseUid_1 index:', dropErr.message);
        }
      }
      try {
        ...
        try {
          ...
          console.warn(' Could not unset null firebaseUid fields:', unsetErr.message);
        }
        ...
        console.warn(' Could not create partial unique index on firebaseUid:', createErr.message);
      }
    } catch (err) {
      console.warn(' Skipping firebaseUid index maintenance:', err.message);
    }
    ...
    console.error('\n  Backend will continue but database features will not work!\n');
